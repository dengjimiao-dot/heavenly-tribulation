...
Future<void> _heroTraceJiejiTalisman(dynamic location) async {
  final isRented = await GameLogic.checkRented(location);
  if (!isRented) return;

  final result = await engine.hetu.invoke('traceJiejiTalisman');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_traceJiejiTalisman_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
  ...
  if (result == 'poor') {
    dialog.pushDialog(
      'hint_traceJiejiTalisman_notEnough',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _heroGuardJiejiResidence(dynamic location) async {
  final isRented = await GameLogic.checkRented(location);
  if (!isRented) return;

  final result = await engine.hetu.invoke('guardJiejiResidence');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_guardJiejiResidence_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
  ...
  if (result == 'poor') {
    dialog.pushDialog(
      'hint_guardJiejiResidence_notEnough',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _heroScourJiejiSite(dynamic location) async {
  final kind = location['kind'];
  final result = await engine.hetu.invoke(
    'scourJiejiSite',
    positionalArgs: [kind],
...
Future<void> _heroDelveJiejiTomb(dynamic location) async {
  final result = await engine.hetu.invoke('delveJiejiTomb');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_delveJiejiTomb_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _heroRaiseJiejiBlackLot(dynamic location) async {
  final result = await engine.hetu.invoke('raiseJiejiBlackLot');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_raiseJiejiBlackLot_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
  ...
  if (result == 'poor') {
    dialog.pushDialog(
      'hint_raiseJiejiBlackLot_notEnough',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _heroCleaveJiejiArray(dynamic location) async {
  final isRented = await GameLogic.checkRented(location);
  if (!isRented) return;

  final result = await engine.hetu.invoke('cleaveJiejiArray');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_cleaveJiejiArray_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
  ...
  if (result == 'poor') {
    dialog.pushDialog(
      'hint_cleaveJiejiArray_notEnough',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _heroDeepJiejiPsychic(dynamic location) async {
  final isRented = await GameLogic.checkRented(location);
  if (!isRented) return;

  final result = await engine.hetu.invoke('deepJiejiPsychic');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_deepJiejiPsychic_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
  ...
  if (result == 'poor') {
    dialog.pushDialog(
      'hint_deepJiejiPsychic_notEnough',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _heroRenewJiejiTheurgy(dynamic location) async {
  final isRented = await GameLogic.checkRented(location);
  if (!isRented) return;

  final result = await engine.hetu.invoke('renewJiejiTheurgy');
  if (result == 'cooldown') {
    dialog.pushDialog(
      'hint_renewJiejiTheurgy_cooldown',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
  ...
  if (result == 'poor') {
    dialog.pushDialog(
      'hint_renewJiejiTheurgy_notEnough',
      npcId: location['npcId'],
    );
    await dialog.execute();
    return;
...
Future<void> _onInteractSite(
  ...
  if (siteKind == 'tradinghouse' || kProductionSiteKinds.contains(siteKind)) {
    ...
    siteOptions.add({
      'text': 'traceJiejiTalisman',
      'description': 'hint_traceJiejiTalisman_description',
    });
    ...
    siteOptions.add({
      'text': 'guardJiejiResidence',
      'description': 'hint_guardJiejiResidence_description',
    });
    if (siteKind == 'home') {
      siteOptions.add({
        'text': 'restJiejiSite',
    ...
    siteOptions.add({
      'text': 'cleaveJiejiArray',
      'description': 'hint_cleaveJiejiArray_description',
    });
    ...
    siteOptions.add({
      'text': 'deepJiejiPsychic',
      'description': 'hint_deepJiejiPsychic_description',
    });
    ...
    siteOptions.add({
      'text': 'renewJiejiTheurgy',
      'description': 'hint_renewJiejiTheurgy_description',
    });
  ...
  switch (selected) {
    ...
    case 'cleaveJiejiArray':
      await _heroCleaveJiejiArray(location);
    case 'castJiejiVeil':
    ...
    case 'deepJiejiPsychic':
      await _heroDeepJiejiPsychic(location);
    case 'invokeJiejiTheurgy':
      await _heroInvokeJiejiTheurgy(location);
    case 'vowJiejiTheurgy':
      await _heroVowJiejiTheurgy(location);
    case 'renewJiejiTheurgy':
      await _heroRenewJiejiTheurgy(location);
    case 'inkJiejiTattoo':
    ...
    case 'traceJiejiTalisman':
      await _heroTraceJiejiTalisman(location);
    case 'recruitJiejiAid':
      await _heroRecruitJiejiAid(location);
    case 'patrolJiejiResidence':
      await _heroPatrolJiejiResidence(location);
    case 'guardJiejiResidence':
      await _heroGuardJiejiResidence(location);
